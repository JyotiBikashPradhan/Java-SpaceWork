...
public class Forgot extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			PreparedStatement ps = cn.prepareStatement("select email from user_registration where email=?");
			...
			if(rs.next())
			{
				...
				PreparedStatement ps1 = cn.prepareStatement("update user_registration set otp=? where email=?");
				ps1.setInt(1, num);
