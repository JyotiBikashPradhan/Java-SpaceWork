<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<link rel="stylesheet" href="css/reportUser.css" type="text/css" media="all" /><!-- Style-CSS -->
<link rel="stylesheet" href="css/loginAdmin.css" type="text/css" media="all" /><!-- Style-CSS -->
	<link href="css/font-login.css" rel="stylesheet">
<meta charset="ISO-8859-1">
<title>Report User</title>
</head>
<body>


<section class="w3l-form-36">
		<div class="form-36-mian section-gap">
			<div class="wrapper">
				<div class="form-inner-cont">
					<h3>User Report Page</h3>
					<form action="ReportUser" method="post" class="signin-form">
					<center>
						<label for="story">Report Administration:</label>

						<textarea id="story" name="report" rows="15" cols="100">
						
						Report Your Complains To The Administration!! Here</textarea></center>
						
						<div class="login-remember d-grid">
							
								
								<span class="checkmark"></span>
								
							</label>
							<button class="btn theme-button">Report</button>
						</div>
					
					</form>
				
					<!-- <p class="signup">Don’t have an account? <a href="re.jsp" class="signuplink">Sign up</a></p> -->
					<p class="signup">Want to go Home Page?  <a href="userHomePage.jsp" class="signuplink">Home Page</a></p>
				</div>

				<!-- copyright -->
				<div class="copy-right">
					<p>© 2020 Active Login Form. All rights reserved ||</p>
				</div>
				<!-- //copyright -->
			</div>
		</div>
	</section>




</textarea>

</body>
</html>